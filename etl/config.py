...
class ScreenConfig:
    ...
    # 是否把法人合計買超列為籌碼面的必要條件。歷史掃描顯示它沒有貢獻：
    # 「融資遞減＋外資庫存增加」20 日超額 +1.02%、贏過市場 54%（樣本 12817），
    # 加上法人買超變成 +1.01%、54%（樣本 7723）——效果相同但樣本少了 66%，
    # 等於只在砍候選數。法人買超單獨的 20 日超額只有 +0.23%、贏過 51%
    # （樣本 50100），因為 5 日買賣超是流量雜訊，而融資與外資是三個月的
    # 部位趨勢，把快訊號混進慢命題只是隨機過濾。
    # 仍會計算並顯示，要嚴格篩選可改為 True。
