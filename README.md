...
**`refresh` 修兩種洞，排程裡已經包含這一步。**

1. **延遲公布**：TWSE 的 `MI_QFIIS`（外資持股）當天查會回「查詢日期大於
   可查詢最大日期」，要等下一個交易日才公布。
2. **當日抓取失敗**：某天抓的時候端點路徑還是錯的，之後修好了，
   但 `backfill` 只補「完全沒有快照」的日期，**部分完成的快照不會被碰到**，
   那一組就永遠是空的。實際遇過：2026-09-08 全市場 1950 檔的融資餘額都是
   空的，而前後幾天都有 1050 檔左右。

不指定 `--datasets` 時，`refresh` 會逐日偵測整組缺漏的資料集，只補真正缺的。
